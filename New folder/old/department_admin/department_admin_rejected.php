                    ...
                    <div class="card-body">
                      ...
                      <div class="col-sm-12">
                        <table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                          ...
                          <tbody id="myTable">
                          <?PHP 
                                  $sql="SELECT * FROM `project_proposals` WHERE  `status` like 'REJECTED%' ORDER BY `sno` DESC ";
                                  $result = mysqli_query($connect,$sql);
                                  $i=1;
                                     while($row = mysqli_fetch_array($result)){
                                      echo "<tr role='row' class='even'><td>".$i."</td><td>".$row['proposal_id']."</td><td>".$row['project_title']."</td>
                                      <td>".$row['estimated_amunt']."</td>
                                      <td style='display:flex;justify-content: center;'><a href='department_admin_rejected_view.php?k=".$_SESSION['Proposal_Id']=$row['proposal_id']."' class='btn btn-primary btn-circle btn-sm'> <i class='far fa-file-alt'></i></a> </td>
                                      </tr>";
                                      $i++;
                                    }
                                ?>
