          ...
          <!--=================================-->
            <div class="card shadow mb-4">
              <div class="card-header py-3">
                      <h6 class="m-0 font-weight-bold text-primary">New Proposals</h6>
               </div>
               <!------------------------>
                    <div class="card-body">
                      ...
                      <div class="col-sm-12">
                        <table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                          ...
                          <tbody id="myTable">
                          <?PHP 
                                  $sql="SELECT * FROM `project_proposals` WHERE  `status` = 'ACCETPED AT DEPARTMENT LEVEL ,PENDING AT CENTRAL COMMITTEE' ORDER BY `sno` DESC ";
                                  $result = mysqli_query($connect,$sql);
                                  $i=1;
                                     while($row = mysqli_fetch_array($result)){
                                      echo "<tr role='row' class='even'><td>".$i."</td><td>".$row['proposal_id']."</td><td>".$row['project_title']."</td>
                                      <td>".$row['estimated_amunt']."</td>
                                      <td style='display:flex;justify-content: center;'><a href='central_committee_pending_view.php?k=".$_SESSION['Proposal_Id']=$row['proposal_id']."' class='btn btn-primary btn-circle btn-sm'> <i class='far fa-file-alt'></i></a> </td>
                                      </tr>";
                                      $i++;
                                    }
                                ?>
